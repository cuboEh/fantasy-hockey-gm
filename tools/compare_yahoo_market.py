"""Build a read-only Yahoo market comparison from user-supplied and cached data."""
import argparse
...
def compare(snapshot,board,catalog):
...
def main():
    ...
    if bool(args.workloads)!=bool(args.rates):parser.error('Supply both workloads and rates')
    if bool(args.context)!=bool(args.case_map) or (args.context and not args.workloads):parser.error('Context needs case map and workload inputs')
    ...
    report=compare(json.loads(args.snapshot.read_text()),json.loads(args.board.read_text()),catalog)
    ...
    if args.workloads:
        ...
        paths.extend(Path('src/fantasy_hockey').glob('*.py'))
